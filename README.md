...
<body class="bg-slate-100 font-sans min-h-screen text-slate-800">
            ...
            <div id="tableSectionContainer" class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-4 border-b border-slate-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-slate-50/50">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Edite as colunas diretamente na tabela. Use os campos no cabeçalho das colunas para filtrar por qualquer coluna.</p>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2 items-center w-full lg:w-auto justify-end no-print">
                        ...
                        <input type="text" id="tableSearchInput" onkeyup="filterTable()" placeholder="Buscar geral..." class="text-xs px-3 py-2 border border-slate-300 rounded-lg bg-white">
    ...
    <script>
        ...
        // RENDERIZAÇÃO DOS CABEÇALHOS DA TABELA COM FILTROS INDIVIDUAIS POR COLUNA
        function renderTableHeaders() {
            const headerRow = document.getElementById('tableHeader');
            const tableThead = document.getElementById('tableThead');
            headerRow.innerHTML = '';

            let filterRow = document.getElementById('tableHeaderFilters');
            if (!filterRow) {
                filterRow = document.createElement('tr');
                filterRow.id = 'tableHeaderFilters';
                filterRow.className = 'bg-slate-700 sticky top-[37px] z-20 border-b border-slate-600';
                tableThead.appendChild(filterRow);
            }
            filterRow.innerHTML = '';

            excelHeaders.forEach((h) => {
                const th = document.createElement('th');
                th.className = 'p-2.5 whitespace-nowrap bg-slate-800 text-white font-bold border-b border-slate-700 text-[11px] uppercase tracking-wider';
                th.innerText = h.toUpperCase();
                headerRow.appendChild(th);

                const thFilter = document.createElement('th');
                thFilter.className = 'p-1.5 bg-slate-700';

                const input = document.createElement('input');
                input.type = 'text';
                input.placeholder = 'Filtrar...';
                input.className = 'w-full text-[10px] px-2 py-1 rounded border border-slate-500 bg-slate-800 text-white focus:outline-none focus:ring-1 focus:ring-indigo-400 placeholder-slate-400 font-normal';
                input.dataset.colHeader = h;
        ...
        // LÓGICA DE FILTRAGEM COMBINADA (BUSCA GERAL + FILTROS DE CABEÇALHO DAS COLUNAS)
        function filterTable() {
            ...
            const colFilterInputs = document.querySelectorAll('#tableHeaderFilters input');
            const colFilters = {};
            colFilterInputs.forEach(input => {
                const val = input.value.trim().toLowerCase();
                if (val) {
                    colFilters[input.dataset.colHeader] = val;
                }
            });
            ...
            const filtered = rawAtivosData.filter(item => {
                ...
                for (const [header, filterVal] of Object.entries(colFilters)) {
                    const cellVal = String(item.originalRow[header] || '').toLowerCase();
        ...
        function resetFilters() {
            ...
            const colFilterInputs = document.querySelectorAll('#tableHeaderFilters input');
            colFilterInputs.forEach(input => input.value = '');
